...
public class Baraja {
	...
	public Baraja() {
		...
		for (int i = 1; i <= 4; i++) {
			for (int j = 2; j <= 14; j++) {
				...
				switch (i) {
				case 1:
					mazo.add(new Carta(valor, "C"));
					break;
				case 2:
					mazo.add(new Carta(valor, "D"));
					break;
				case 3:
					mazo.add(new Carta(valor, "P"));
					break;
				case 4:
					mazo.add(new Carta(valor, "T"));
					break;
				}
			}
		}
	...
	private void asignarImagen() {
		try {
			int index = 0;
			BufferedImage imagenesCartas = ImageIO.read(getClass().getResource("/recursos/cards.png"));
